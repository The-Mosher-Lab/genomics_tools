...
ggplot(data = readlengths, aes(x = V2, y = V1)) +
  geom_line(size = 2, color = "red") +
  theme(plot.title = element_text(size = 30)) + # Change axis titles
  ylab("Reads") + # \n adds a new line, increases distance between label and axis if desired
  xlab("Length (nt)") +
...
# Saves output to .png based on input filename without .txt extension, w and h may be changed as desired
