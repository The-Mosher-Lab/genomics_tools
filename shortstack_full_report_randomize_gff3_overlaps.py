...
def overlap_randomized_clusters_gff(gff_dict, randomized_clusters,
    ...
    for chromosome in gff_dict:
        if chromosome in randomized_clusters:
            for feature_id in gff_dict[chromosome]:
                feature_start = gff_dict[chromosome][feature_id][0]
                feature_stop = gff_dict[chromosome][feature_id][1]
...
def bootstrapper(loci_tsv, fasta_file, gff_file, gff_feature, upstream_bp,
    ...
    gff_features = parse_gff(gff_file, gff_feature)
    ...
    with open(output_file, 'w') as output_handle:
        ...
        for i in range(bootstraps):
            ...
            overlaps = overlap_randomized_clusters_gff(
                gff_features, randomized_clusters, upstream_bp,
...
parser = ArgumentParser(
    description='Randomize shortstack loci and look for overlap with gff3 '
    'features')
parser.add_argument(
...
output_file = gff_file.rsplit('.gff', 1)[0] + (
...
bootstrapper(loci_tsv, fasta_file, gff_file, gff_feature, upstream, downstream, body,
